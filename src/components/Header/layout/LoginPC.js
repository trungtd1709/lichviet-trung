...
function FormatDate(d, format = "") {
  ...
  if (typeof date === "string") {
    ...
    if (Array.isArray(formatDate)) {
      if (formatDate[0].length === 4) {
        let date1 =
          formatDate[1].length === 1
            ? "0" + parseInt(formatDate[1])
            : formatDate[1];
        let date2 =
          formatDate[2].length === 1
            ? "0" + parseInt(formatDate[2])
            : formatDate[2];
        date = formatDate[0] + "-" + date1 + "-" + date2;
      } else {
        let date1 =
          formatDate[0].length === 1
            ? "0" + parseInt(formatDate[0])
            : formatDate[0];
        let date2 =
          formatDate[1].length === 1
            ? "0" + parseInt(formatDate[1])
  ...
  let DAY =
    parseInt(dateFormat.getDate()) < 10
      ? "0" + parseInt(dateFormat.getDate())
      : parseInt(dateFormat.getDate());
  let MONTH =
    parseInt(dateFormat.getMonth() + 1) < 10
      ? "0" + parseInt(dateFormat.getMonth() + 1)
      : parseInt(dateFormat.getMonth() + 1);
  const doi_ngay = {
    0: "Chủ nhật",
    1: "Thứ hai",
    2: "Thứ ba",
  ...
  let objDate = {
    ...
    H:
      Number(dateFormat.getHours()) < 10
        ? "0" + dateFormat.getHours()
        : dateFormat.getHours(),
    i:
      Number(dateFormat.getMinutes()) < 10
        ? "0" + dateFormat.getMinutes()
        : dateFormat.getMinutes(),
    s:
      Number(dateFormat.getSeconds()) < 10
          ...
          <div className={"modal-user-detail"}>
            <div className={"user-info d-flex"}>
              ...
              <div className={"user-detail"}>
                ...
                <p>ID: {user.id}</p>
                <p>
                  {checkPremium ? "Thành viên PRO" : "Chưa phải thành viên PRO"}
                </p>
              </div>
            </div>
                    ...
                    return (
                      <div className="item-package" key={key}>
                        <div className={"icon-package"}>
                          <img
                            src={"https://cdn.lichviet.org" + item.thumb}
                            alt=""
                            width={35}
                          />
                        </div>
                        <div className={"detail-package"}>
                          <p className={"package-name"}>
                            {mapPackageName[item.premium_groups]}
                          </p>
                          <p className={"hsd"}>
                            HSD: {FormatDate(item.end_time, "H:i • d/m/Y")}
                          </p>
                        </div>
                      </div>
                    );
            ...
            <div className={"menu-control"}>
              ...
              <div className={"item-menu"} onClick={logout}>
                <img
                  src={BASE_URL_IMAGE + "/images/icon/icon-logout.png"}
                  width={35}
                  alt=""
                />
                <p>Đăng xuất</p>
